...
if __name__ == '__main__':
    ...
    # transform
    #camera_parameter=[640,260,640,640,320,130]
    ...
    kitti_dataset_test = data.data_loader.SepeDataset(path_to_poses_files=motion_files_path_test,path_to_image_lists=path_files_path_test,transform_=transforms_,norm_flag=1,camera_parameter= camera_parameter,coor_layer_flag = coor_layer_flag,color_flag=color_flag)

    dataloader_vid = DataLoader(kitti_dataset_test, batch_size=input_batch_size,shuffle=False ,num_workers=1,drop_last=False)
    ...
    np.savetxt('../saved_new/'+args.model_name+'_path_result.txt',forward_visual_result_m)
    np.savetxt('../saved_new/'+args.model_name+'_path_result_opti.txt',forward_visual_opti_m)
    np.savetxt('../saved_new/'+args.model_name+'_reli_error.txt',reliability_error)
    ...
    #vis.plot_path_with_gt(forward_visual_opti_m,ground_truth_m,17,'testing set forward')
    print(evaluate.evaluate(ground_truth_m,forward_visual_opti_m))
    print(np.mean(evaluate.evaluate(ground_truth_m,forward_visual_opti_m),1))
    forward_visual_opti_m = np.loadtxt('../saved_new/'+args.model_name+'_path_result_opti.txt')
    print(evaluate.evaluate(ground_truth_m,forward_visual_opti_m))
    print(np.mean(evaluate.evaluate(ground_truth_m,forward_visual_opti_m),1))
