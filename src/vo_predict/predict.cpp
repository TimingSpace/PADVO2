...
PYBIND11_MODULE(egomotionprediction,m)
{   
    py::class_<EgoMotionPrediction>(m,"EgomotionPrediction")
        .def(py::init<>())
        .def("predict",&EgoMotionPrediction::predict)
        .def("predict_group",&EgoMotionPrediction::predict_group)
        .def("predict_patch",&EgoMotionPrediction::predict_patch);
}
